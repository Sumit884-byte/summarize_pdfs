...
def test_canonical_facts_not_injected():
    """Branch facts come from LLM output, not hardcoded injection."""
    assert CANONICAL_TOPIC_FACTS == {}
    assert canonical_facts_for_topic("Descriptive Statistics") == []


def test_notes_render_key_facts_section():
    bucket = _NotesBucket("Descriptive Statistics")
    bucket.add_fact("Sample statistics estimate population parameters")
    bucket.add_definition("mean", "the arithmetic average of a set of numeric values")
    text = "\n".join(_render_notes_topic(bucket))
    assert "Key Facts:" in text
    assert "Def:" in text
    assert "population" in text.lower() or "sample" in text.lower()


def test_summary_render_key_facts_section():
    bucket = _TopicBucket("Probability & Conditional Probability")
    bucket.add_fact("Probability always ranges from 0 to 1")
